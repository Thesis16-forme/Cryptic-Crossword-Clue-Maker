    ...
    <div>
      ...
      <div className="relative">
        <input
          ...
          value={value}
          maxLength={maxLength}
          className={`w-full bg-gray-700 border border-gray-600 rounded-md shadow-sm py-3 px-4 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition ${onSuggestClick ? 'pr-12' : ''} ${isHighlighted ? 'highlight-animation' : ''}`}
          {...props}
        />
        {onSuggestClick && (
          <button
